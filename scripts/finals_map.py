...
IDX = os.path.join(ROOT, '.tmp', 'burn2026', 'data', 'finals_index.json')
...
def main():
    ...
    json.dump({k: v for k, v in m.items()}, open(os.path.join(ROOT, '.tmp', 'burn2026', 'data', '_finals_txtmap.json'), 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
