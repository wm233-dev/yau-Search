...
P,R,D = "P","R","D"
SPEC = [
 ("2010","2010_Applied_Computational_Probability_and_Statistics_individual.txt","individual",4,0,D),
...
VERBS=["prove","show","find","compute","derive","calculate","determine","evaluate","construct","justify","disprove","describe","verify","explain","suggest","identify","compare","discuss","estimate","argue","give","solve"]
rows=[]
for (y,f,kind,exp,skip,style) in SPEC:
    t=slice_ps(f,read(os.path.join(TXT,f)))
    ms=[(int(m.group(1)),m.start(),m.end()) for m in RX[style].finditer(t)]
    if len(ms)<exp+skip: print("!! WARN",f,"only",len(ms))
...
print(f"{'yr':5s}{'n':>4s}{'meanCh':>8s}{'medCh':>7s}{'maxCh':>7s}{'meanSub':>9s}{'prove':>7s}{'show':>6s}{'find':>6s}{'comp':>6s}{'der':>5s}{'det':>5s}")
for y in sorted(by):
    rs=by[y]
    print(f"{y:5s}{len(rs):4d}{st.mean(r['chars'] for r in rs):8.0f}{st.median(r['chars'] for r in rs):7.0f}{max(r['chars'] for r in rs):7d}{st.mean(r['subparts'] for r in rs):9.2f}"
...
for name,ys in [("2010-2012",{"2010","2011","2012"}),("2013-2015",{"2013","2014","2015"}),("2016-2018",{"2016","2017","2018"}),("2019-2021",{"2019","2020","2021"}),("2022-2026",{"2022","2023","2024","2025","2026"}),("2020-2026",{"2020","2021","2022","2023","2024","2025","2026"}),("ALL",set(by))]:
    rs=[r for r in rows if r['year'] in ys]
    print(f"{name}: n={len(rs)} mean={st.mean(r['chars'] for r in rs):.0f} med={st.median(r['chars'] for r in rs):.0f} max={max(r['chars'] for r in rs)} meanSub={st.mean(r['subparts'] for r in rs):.2f} prove={sum(r['verbs']['prove'] for r in rs)} show={sum(r['verbs']['show'] for r in rs)} find={sum(r['verbs']['find'] for r in rs)} compute={sum(r['verbs']['compute'] for r in rs)} derive={sum(r['verbs']['derive'] for r in rs)} determine={sum(r['verbs']['determine'] for r in rs)} calculate={sum(r['verbs']['calculate'] for r in rs)}")
json.dump(rows, open(r"E:\deepseek_exclusive\math\.tmp\burn2026\scripts\prob_ps_rows_final.json","w"), ensure_ascii=False, indent=1)
for r in sorted(rows,key=lambda r:r['chars'])[:3]: print("SMALL",r['year'],r['n'],r['chars'],repr(r['head'][:60]))
for r in sorted(rows,key=lambda r:-r['chars'])[:3]: print("BIG  ",r['year'],r['n'],r['chars'],repr(r['head'][:60]))
