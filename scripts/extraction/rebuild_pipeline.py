...
"""
import os, io, re, json, html, itertools, collections, statistics, sys

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from _paths import (REPO_ROOT, DATA as _DATA, REPORTS as _REPORTS, CORPUS_PRELIM)

ROOT = str(REPO_ROOT)          # reads corpus/prelim/, writes data/ and reports/
TXT, DATA, REP = str(CORPUS_PRELIM), str(_DATA), str(_REPORTS)
...
L = ["# Yau 初赛(笔试) 语料库统计 v3（单一流水线 rebuild_pipeline.py 生成）\n",
     "数据源：" + BT + "sources/prelim" + BT + "（136 个 PDF）→ PyMuPDF 抽取 → 统一解析。",
