...
o()
o('> 语料：BT./corpus/finals/BT（195 个 txt，源自 BT sources/finals BT）')
o('> 加上本报告新增恢复的 BT./corpus/finals_extra/BT。')
o('> 元数据：BT./data/finals_index.jsonBT；txt 与原卷的映射由 BT./scripts/finals_map.pyBT 建立')
...
o('- 上述 3 卷已用 PyMuPDF 以 200 dpi 渲染为 PNG，再由视觉逐字识读（脚本 BTrender_2015_alg.pyBT，产物 BT./data/render/BT），')
...
o()
o('恢复脚本：BT./scripts/extract_extra_finals.pyBT；两个 BT.docBT 用 Word COM **只读打开、只写工作区**（未改动 F 盘任何文件）。')
o('产物目录：BT./corpus/finals_extra/BT。它们的价值：把分析科目的年份覆盖从「2015 起」提前到 **2012**，')
