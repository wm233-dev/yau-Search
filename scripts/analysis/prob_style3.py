...
import re, os, json
src=open(r".\scripts\prob_twins2.py",encoding='utf-8').read()
exec(src.split("keys=list(store)")[0])
json.dump({f"{k[0]}|{k[1]}|{k[2]}":v for k,v in store.items()}, open(r".\scripts\prob_ps_bodies.json","w"), ensure_ascii=False, indent=1)
