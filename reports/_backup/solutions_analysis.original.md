...
> **数据来源**：<code>.tmp/burn2026/data/problems_full.json</code>（全文结构化题库；截至本讲义定稿为 <b>751</b> 条记录，其中 subject 为 "Analysis & PDE" 者 <b>155</b> 道，横跨 2010–2026 共 17 年）。
...
- **交叉重复考点提醒**：本题库中 **2021 Individual 第 2 题、2022 Individual 第 4 题、2026 Individual 第 4 题是同一道题**（"$C[0,1]$ 中由多项式组成的闭线性子空间必有限维"）。这是 17 年里罕见的"原题三度出现"。本讲义未选它（重复出题意味着区分度低），但备考时务必会做。
...
- 题库中"问题粘连"的年份（2010、2012、2019、2020、2024）在选题时格外小心：例如 2010 Individual 第 2 题的文本开头其实是第 1(b) 题的尾巴，2012 Team 第 1 题混入了第 2 题，2020 Individual 第 3 题混入了第 4 题。这些题目一律弃用，详见 §12.2。
...
$$C\Big(x^{a_0-1}+x^{a_1-1}\Big)\frac{|\log x|}{1+x^2}\cdot\frac{1}{1+x^2}\quad\text{型控制函数}$$
控制（分 $(0,1]$ 与 $[1,\infty)$ 两段分别估计），于是由 **Leibniz 积分号下求导定理**，
...
若要用法三，**不要**对 $\frac{\log z}{1+z^2}$ 直接用 keyhole——$\log$ 在支割两岸的相位记账极易出错。正确做法是考虑 $\frac{(\log z)^2}{1+z^2}$：它的 $\log^2$ 项在两岸相减后留下 $-4\pi i\log x+4\pi^2$ 两部分，分别给出 $\int_0^\infty\frac{\log x}{1+x^2}dx$ 与 $\int_0^\infty\frac{(\log x)^2}{1+x^2}dx$。留数计算
$$\operatorname{Res}_{z=i}\frac{(\log z)^2}{1+z^2}=\frac{(i\pi/2)^2}{2i}=-\frac{\pi^2}{8i},\qquad
\operatorname{Res}_{z=-i}\frac{(\log z)^2}{1+z^2}=\frac{(-i\pi/2)^2}{-2i}=\frac{\pi^2}{8i},$$
两者之和为 $0$，故围道积分为 $0$，从而 $\int_0^\infty\frac{\log x}{1+x^2}dx=0$（同时可得 $\int_0^\infty\frac{(\log x)^2}{1+x^2}dx=\frac{\pi^3}{8}$）。**法一、法二更干净，法三留作复分析练习。**
...
- **但"逼近 + 稠密性"这一骨架反复出现**：
  - **2021 Individual 第 2 题 / 2022 Individual 第 4 题 / 2026 Individual 第 4 题**（**同一道题三年重出**）：$C[0,1]$ 中由多项式构成的闭线性子空间必有限维——用的正是"局部一致有界 + 等度连续（Ascoli–Arzelà）"或"多项式零点集"论证。
...
> **关于 $x=0$ 的细节：**若 $x(t_1)=0$ 对某个 $t_1$ 成立，则 $t\mapsto0$ 是该初值下的一个解，由唯一性 $x\equiv0$，结论平凡。所以在 $x\not\equiv0$ 的情形下，集合 $\{t:x(t)=0\}$ 无内点，(4.1) 在每个开分量上按通常意义（或在零点处按 Dini 导数）成立。
>
> 也提醒：若改用
...
- **2019 Individual 第 3 题**：$\mathbb R^2$ 上调和且 $\lim_{|x|\to\infty}|f(x)|\ln|x|=0$ $\Rightarrow$ $f$ 为常数——同一刚性家族的"远端"版本。
...
**（关键：$t_*=T_{1/2}$，$x$ 不会"提前"停下或跑出 $(-x_0,x_0)$。）** 由 $x$ 的连续性可定义 $x(T_{1/2})=-x_0$；再由 (7.2)，$x'(T_{1/2})=\pm\sqrt{2\big(E_0-V(-x_0)\big)}=0$。
...
写 $\partial_r\varphi(x)=\nabla\varphi(x)\cdot\hat x$。由条件 (a)，$\int_{\partial B_\epsilon}W\,d\sigma=0$，故可把 $\nabla\varphi(x)$ 换成 $\nabla\varphi(0)$：
$$\int_{\partial B_\epsilon}W\,\partial_r\varphi\,d\sigma=\int_{\partial B_\epsilon}W\big(\partial_r\varphi-\nabla\varphi(0)\cdot\hat x\big)d\sigma$$
（因为 $\int_{\partial B_\epsilon}W(\nabla\varphi(0)\cdot\hat x)\,d\sigma=0$——这是 $W$ 乘一个常向量场与 $\hat x$ 的内积，而 $\int_{\partial B_\epsilon}W=0$ 使该项逐分量消失）。由 $\varphi\in C_c^\infty$ 有 $\big|\partial_r\varphi(x)-\nabla\varphi(0)\cdot\hat x\big|\le C_\varphi\,\epsilon$（$|x|=\epsilon$），故
$$\Big|\int_{\partial B_\epsilon}W\partial_r\varphi\,d\sigma\Big|\le C_\varphi\,\epsilon\int_{\partial B_\epsilon}|W|\,d\sigma\le C_\varphi\epsilon\cdot\omega_n\epsilon^{n-1}\cdot C_W\epsilon^{2-n}=C'\epsilon^{2}\xrightarrow[\epsilon\to0]{}0 .$$
（第二个不等号用了 (b)：$|W|\le C_W|x|^{2-n}$ 于 $|x|=\epsilon$。）
...
$$\int_{\partial B_\epsilon}\varphi\,\partial_rW\,d\sigma=\int_{\partial B_\epsilon}\big(\varphi(x)-\varphi(0)\big)\partial_rW\,d\sigma\le\|\nabla\varphi\|_\infty\,\epsilon\int_{\partial B_\epsilon}|\partial_rW|\,d\sigma$$
...
=s\cdot\frac{1}{r^{n-1}}\bigg|_{r=1/s}\int_{\partial B_r}W\,d\sigma=\frac{s}{\omega_n}\cdot 0=0 .$$
...
- **2019 Individual 第 3 题**：$\mathbb R^2$ 上调和且 $\lim_{|x|\to\infty}|f(x)|\ln|x|=0$ $\Rightarrow$ $f$ 为常数（题目要求"证明或举反例"）。
...
（数值核对，$n=3$（此时 $C_{\rm opt}=4$）：Gauss–Legendre 2000 点算得 $\epsilon=0.2,0.1,0.05,0.02,0.01$ 时比值分别为 $11.111,\ 6.250,\ 4.938,\ 4.340,\ 4.165$，单调下降趋向 $4$ ✔）
...
| **题 2（2014 team #1）** | 法二（参数积分）中"积分号下求导"的控制函数我只作了定性描述。 | 写法紧凑度 | 分 $(0,1]$ 与 $[1,\infty)$ 两段用 $x^{a_0-1}+x^{a_1-1}$ 型控制即可，细节是例行估计。法一（反演代换）是**完全自足**的解答，不依赖此步。 |
...
| **2020 Individual #3** | $\mathbb R^4$ 中 $-\Delta u\le u^2$、$\|u\|_{L^2(B_1)}\le\epsilon\Rightarrow\|\nabla u\|_{L^2(B_{1/2})}\le10^4\|u\|_{L^2(B_1)}$（$\epsilon$-正则性） | **题库把第 3 题与第 4 题的文本粘连**（第 3 题末尾混入了 "Problem 4. Let f and g be two holomorphic functions..."），且 "<code>where △= Σ ∂²/∂x_i²</code>" 一行被切到第 4 题里 | **可补救**：原 PDF 在 <code>2020\Analysis&DifferentialEquations\analysis_and_differential_20.pdf</code>，切分后即可用。本讲义因已有 10 道更干净的题而未处理 |
...
| **2021 Individual #2 / 2022 Individual #4 / 2026 Individual #4** | $C[0,1]$ 中由多项式组成的闭线性子空间必有限维 | **题面完整无缺**，放弃原因是**内容重复**：这是一道题在三个年份的原题重出。本讲义只选"不重复"的题，故未收录 | 无需补救——但备考必做。做法提示：设 $\dim P=\infty$，取 $P$ 中线性无关的多项式列并在 $P$ 内做规范化，用有限维空间上范数等价 + 一致有界导出等度连续（Ascoli–Arzelà），再取极限得非多项式元素，与 $P$ 闭矛盾 |
...
3. **题 2 的"法三（围道积分）"在最初草稿中出过相位记账的错误**，我已删除错误版本、改写成对 $(\log z)^2$ 的正确处理，并在正文中标注"法一、法二才是正式解答"。这是一个我主动自曝的推导瑕疵。
...
- 题库文件：<code>.tmp/burn2026/data/problems_full.json</code>（截至定稿为 751 条记录；字段：<code>year / subject / paper / kind / n / chars / text</code>）。**注意：本讲义撰写期间该文件被重新生成过一次（早期快照为 757 条、该科目 156 道），本讲义所有引用均已按定稿快照复核。**
...
- 数值验算（本讲义的全部数值结论均由脚本独立算出，不是抄来的）：
  ...
  - **题 9**：$n=3$ 时比值在 $\epsilon=0.2,0.1,0.05,0.02,0.01$ 处分别为 $11.111,\ 6.250,\ 4.938,\ 4.340,\ 4.165$。
...
1. **问题边界偏移**：2010 Individual 第 1/2 题、2012 Team 第 1/2 题、2014 Team 第 1/2 题、2019 Individual 第 1/2 题、2020 Individual 第 3/4 题——这些题目的 <code>text</code> 字段跨越了题号边界，直接用 <code>text</code> 引用会出现"题目张冠李戴"。
...
4. **原题重出**：2021 Individual #2 = 2022 Individual #4 = 2026 Individual #4；2015 Individual #1 与 2018 Individual #1(2) 也高度同源（"$L^2$ 范数收敛 + 弱收敛 $\Rightarrow$ 强收敛"）；2015 Individual #5 与 Team #5 是同一年两卷的同一道 Fredholm 题。做"考点频次统计"时应先做去重，否则会高估某些考点的权重。
...
6. **题库在撰写期间被重新生成**：本讲义第一次读取时题库为 757 条（该科目 156 道），定稿时为 751 条（该科目 155 道）；被合并/删除的主要是若干"边界偏移"的碎片记录。因此**凡是引用本题库条数、题号的地方，都应以当次快照为准**；本讲义已在 §0、§12.2、附录 B 中统一为定稿快照的数字，并在此明确记录两次快照的差异。
7. **跨卷复用的题目**：2014 Individual 第 3 题（"任何定义在环域 $\{r<|z|<R\}$ 上的有界解析函数 $F$ 都可写成 $F(z)=z^{\alpha}f(z)$"）与 2014 Team 第 3 题是**同一道题**——这是 2014 年两卷之间的一次题目复用。类似情况在做频次统计时同样需要去重。
