...
>
> **高风险题（建议自己复算后再背）**
> - **第 10 题**：Synge 主证明可信；但"三个条件都必需"里"去掉偶维"的反例原先写错（$S^1\times S^2$ 并无正曲率乘积度量，混合平面截面曲率为 0，且 Myers 定理排除之），已改为 $\mathbb{RP}^3$ / 透镜空间 $S^3/\mathbb Z_p$。此处最容易背错。
> - **第 8 题**：正文四步可信；"反向补充"必须附加 $\mu k-\lambda\tau\not\equiv0$，否则沿主法线平移得到的曲线退化为直线。
> - **第 4 题**：答案 $\operatorname{arccosh}\frac32$ 已数值复核（含扰动最优性检验）；但题面 "the complete metric" 本身不严谨，作答须先声明"与标准共形结构相容"。
...
156 道真题中，纯代数拓扑（同调、同伦、补空间、覆叠）约六成，经典曲面/曲线论约两成半，Riemann 几何约一成半。

> ✅ 已按 referee_geometry.md 修正（原表述：159 道真题中……——几何与拓扑题数按当前 problems_full.json 为 156）
...
**第 1 步（引理：覆叠变换判据）.** 设 $p:\widetilde M\to M$ 是覆叠映射，$\widetilde M$ **连通且可定向**，$G$ 为其覆叠变换群。则

> $M$ 可定向 $\iff$ $G$ 中每个元素都保持 $\widetilde M$ 的定向。

> ✅ 已按 referee_geometry.md 修正（原表述：只假设"$p:\widetilde M\to M$ 是 $n$ 维连通流形 $M$ 的一个覆叠映射"，缺"$\widetilde M$ 可定向"；否则恒同覆叠 + Möbius 带即反例）
...
(⇐) 设每个 $\tau\in G$ 都保持 $\widetilde M$ 的一个固定定向（$\widetilde M$ 可定向是引理的假设，故可任取一点处的定向沿道路传播）。

> ✅ 已按 referee_geometry.md 修正（原表述："$\widetilde M$ 可定向：任取一点处的定向沿道路传播即可，$\widetilde M$ 单连通故无单值性问题"——可定向性应由引理假设提供）

对 $x\in M$ 与 $\widetilde x\in p^{-1}(x)$，把 $T_{\widetilde x}\widetilde M$ 的定向经 $p_*$ 推前得到 $T_xM$ 的定向；若换 $\widetilde x'=\tau(\widetilde x)$，因 $\tau$ 保定向，推前结果相同，故良定义。局部上 $p$ 是微分同胚，故得到 $M$ 上处处非零的 $n$-形式。∎
...
因 $A=-I_{n+1}$ 把 $S^n$ 在 $x$ 处的外法向 $x$ 送到 $-x$，而 $-x$ 恰是 $S^n$ 在 $-x$ 处的外法向，故按"外法向 + 切向基"的定向约定，$A$ 在 $T_xS^n$ 上诱导的定向符号等于 $\det(-I_{n+1})=(-1)^{n+1}$（等价写法：$A=R_1\circ\cdots\circ R_{n+1}$ 是 $n+1$ 个坐标反射之积，每个 $R_i$ 都反 $S^n$ 的定向），于是
$$A \text{ 保定向}\iff \det(-I_{n+1})=(-1)^{n+1}>0\iff n\ \text{为奇数}.$$

> ✅ 已按 referee_geometry.md 修正（原表述："它在切空间上的微分同为 $-I_{n+1}$，于是 $A$ 保定向 $\iff\det(-I_{n+1})>0$"——微分作用在 $n$ 维切空间上，需先用外法向约定解释符号）
...
而 $(a,b)\mapsto(Sa)\times(Sb)$ 与 $(a,b)\mapsto\det(S)(a\times b)$ 都是双线性映射，二者在 $(e_1,e_2)$ 上相等（即上式），在退化组合 $(e_1,e_1)$、$(e_2,e_2)$ 上同为 $0$，故恒等：对任意 $a,b\in T_pS$ 有 $(Sa)\times(Sb)=\det(S)(a\times b)$。

> ✅ 已按 referee_geometry.md 修正（原表述："$X\mapsto SX$ 与 $a\times b$ 都是双线性的，故……"——双线性只给"在基上相等"，还须点明 $(e_1,e_1)$、$(e_2,e_2)$ 两个退化组合）

取 $a=\varphi_u,b=\varphi_v$ 并注意 $\det S=K$：
...
> ✅ 已按 referee_geometry.md 修正（原表述："等号成立当且仅当 $N$ 在 $A$ 上单射（重数处处为 1）"——应为"重数几乎处处为 1"）
...
**前提（必须显式写出）.** 题面 "the complete Riemannian metric … with constant curvature $-1$" 须理解为**与 $D^*$ 的标准共形结构相容**的那个完备常曲率 $-1$ 度量（单值化度量；等价说法：两端都是尖点）。否则命题不真——环带 $\cong D^*$ 上还存在含核心闭测地线的"漏斗型"完备双曲度量（例如 $\mathbb H/\langle z\mapsto 2z\rangle$ 经任一微分同胚 $D^*\to\mathbb H/\langle 2z\rangle$ 推前），核心闭测地线长度 $\ell>0$ 可连续变化，给出互不等距的一族完备常曲率 $-1$ 度量，同一对点的距离随之改变。

> ✅ 已按 referee_geometry.md 修正（原表述：直接以"the complete Riemannian metric"的口吻使用唯一性——须先声明"与标准共形结构相容"，否则答案不唯一）
...
**为什么必须是偶维**：若 $n$ 为奇数，则 $(1\otimes\alpha)(\alpha\otimes1)=(-1)^{n^2}\alpha\otimes\alpha=-\alpha\otimes\alpha$，两项相消，第 2 步得不到矛盾——这与 $S^1,S^3,S^7$ 确实是 H-空间（$\mathbb C,\mathbb H,\mathbb O$ 的单位球）相符 ✓。完整分类（$S^n$ 是 H-空间 $\iff n\in\{0,1,3,7\}$；$S^0=\{\pm1\}$ 本身是拓扑群，官方 2022 解答的 Remark 亦写作 $S^0,S^1,S^3,S^7$）即 Adams 的 Hopf 不变量一（深度定理，本讲义不证）。

> ✅ 已按 referee_geometry.md 修正（原表述：$S^n$ 是 H-空间 $\iff n\in\{1,3,7\}$——漏了 $n=0$）
...
**补充（这条恒等式的一般含义）.** $N^*g_{S^2}=-K\,ds^2$ 正是"极小曲面的高斯映射（反）共形"的精确表述，而且保角因子恒为 1——即 $N$ 在这里是等距而非仅仅共形。当 $K>0$ 时 $-K\,ds^2$ 负定、**不是**度量；此时应改看 $|K|\,ds^2$（正定），它等于 $N^*g_{S^2}$ 复合 $S^2$ 的反向定向（即把 $S^2$ 换成反向定向的球面），结论相应地变成 $\widetilde K=-1$；本题的 $K<0$ 恰使 $-K\,ds^2$ 正定，这正是命题能这样陈述的原因。

> ✅ 已按 referee_geometry.md 修正（原表述："当 $K>0$ 时 $-K\,ds^2$ 负定，需把 $S^2$ 反向定向……结论变成 $\widetilde K=-1$"——负定张量不是度量，应改看 $|K|\,ds^2$）
...
**补充（反向与特例）**：反过来，若 $C$ 的 $k,\tau$ 处处非零、存在常数 $\lambda,\mu$ 使 $\lambda k+\mu\tau\equiv1$，**且 $\mu k-\lambda\tau\not\equiv0$**，则把 $C$ 沿主法线方向平移 $\lambda$ 所得的曲线 $D$ 与 $C$ 构成 Bertrand 对：此时 $D'=\tau(\mu t+\lambda b)$、$D''=\tau'(\mu t+\lambda b)+\tau(\mu k-\lambda\tau)n$，其曲率法向分量为 $\tau(\mu k-\lambda\tau)\neq0$，故 $D$ 的主法线为 $\pm n$。

**退化情形（必须排除）**：若 $\mu k\equiv\lambda\tau$，与 $\lambda k+\mu\tau\equiv1$ 联立解得 $(\lambda,\mu)=\dfrac{(k,\tau)}{k^2+\tau^2}$（此时 $k,\tau$ 必为常数），此时 $D''\equiv0$，平移所得的 $D$ 是一条**直线**：曲率为 $0$、主法线不存在，**不构成** Bertrand 对。（数值例：$k=0.596330,\ \tau=0.321101$ 的螺旋取 $\lambda=1.3,\ \mu=0.7$ 时平移曲线最大曲率 $\approx2\times10^{-7}$；取 $(\lambda,\mu)=(1/k,0)$ 时曲率 $0.596330>0$，是合法对。）

特别地 $\mu=0$ 对应 $k\equiv1/\lambda$ 为常数（此时 $\mu k-\lambda\tau=-\lambda\tau\not\equiv0$，非退化），这说明"每个常曲率的空间曲线都是 Bertrand 曲线"。

> ✅ 已按 referee_geometry.md 修正（原表述："反过来……存在常数 $\lambda,\mu$ 使 $\lambda k+\mu\tau=1$，则平移 $\lambda$ 即得 Bertrand 对"——缺 $\mu k-\lambda\tau\not\equiv0$；否则平移曲线退化为直线）
...
*(ii) 同伦推出（自足论证）.* $N=P\times[-\varepsilon,\varepsilon]$ 是亏格为 $b(P)-1=3$ 的柄体（$P$ 有 4 个边界圆、$\chi(P)=-2$，$P\times I$ 的亏格 $=1+(-\chi(P))-1=3$ ✓）。更严格地：$Y=B_1\cup_A B_2$，其中 $A=\bigsqcup_{i=1}^{4}D_i$ 是可缩子复形、$A\hookrightarrow B_i$ 是余纤维化，故 $Y$ 是 $A\hookrightarrow B_1$、$A\hookrightarrow B_2$ 的**同伦推出**；把 $B_i\simeq$ 点代入得
$$Y\ \simeq\ \Sigma A\ \simeq\ \Sigma(4\ \text{点})\ =\ S^1\vee S^1\vee S^1 ,$$
于是 $\pi_1(Y)=F_3$、$H_1(Y;\mathbb Z)=\mathbb Z^3$，与 Alexander 对偶一致 ✓。**注意**：不要用"柄体的外部仍是柄体"这一命题——它依赖嵌入方式（把实心环按三叶结嵌入 $S^3$ 时其外部是三叶结补，$\pi_1\cong\langle a,b\mid a^2=b^3\rangle\neq\mathbb Z$），本题必须用上面的同伦推出或第 3 步的广群 van Kampen。

> ✅ 已按 referee_geometry.md 修正（原表述："$N$ 是标准柄体，其在 $S^3$ 中的外部也是亏格 3 的柄体"——该概括一般不真，已改为同伦推出 $Y\simeq S^1\vee S^1\vee S^1$）
...
- 去掉"偶维"：取奇维球面商，例如 $\mathbb{RP}^3=S^3/\{\pm1\}$，或更一般的透镜空间 $L(p;1)=S^3/\mathbb Z_p$：紧、可定向、带圆球商度量时截面曲率恒 $>0$，而 $\pi_1=\mathbb Z_2$（或 $\mathbb Z_p$）$\neq1$；
  （**不能**取 $S^1\times S^2$：乘积度量下由一条 $S^1$ 方向与一条 $S^2$ 方向张成的平面截面曲率**恒为 0**，并非正曲率；而且由 Myers 定理，正截面曲率 $\Rightarrow$ 正 Ricci 曲率 $\Rightarrow\pi_1$ 有限，而 $\pi_1(S^1\times S^2)=\mathbb Z$，故 $S^1\times S^2$ 上**不存在**正截面曲率度量。）

> ✅ 已按 referee_geometry.md 修正（原表述："去掉偶维：$S^1\times S^2$ ……有正曲率乘积度量"——混合平面截面曲率为 0，Myers 定理更排除其正曲率度量）
